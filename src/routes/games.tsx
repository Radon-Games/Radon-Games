...
export default function Games(): JSX.Element {
  ...
  const [gameCards, setGameCards] = createSignal<Game[]>(
    sortedGames.slice(0, 20)
  );

  function createObserver(element: HTMLElement) {
    let interval = setInterval(() => {
      if (checkVisible(element)) {
        if (gameCards().length === sortedGames.length) clearInterval(interval);
        let y = document.documentElement.scrollTop;
        setGameCards(
          gameCards().concat(
            sortedGames.slice(gameCards().length, gameCards().length + 20)
          )
        );
  ...
  return (
    <main>
      ...
      <div class="px-8 sm:px-16 md:px-20 lg:px-32 grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {gameCards().map((game: Game): JSX.Element => {
        ...
        <Show when={gameCards().length !== sortedGames.length}>
          <SkeletonGameCard ref={createObserver} />
          <SkeletonGameCard />
          <SkeletonGameCard />
          <SkeletonGameCard />
        </Show>
